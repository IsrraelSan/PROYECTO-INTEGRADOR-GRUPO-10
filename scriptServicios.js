...
async function getWeather() {
    ...
    try {
        const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${apiKey}&lang=es`);
        ...
        const temperatureRange = `${toCelsius(minTemperature)}°C - ${toCelsius(maxTemperature)}°C`;
        ...
        alert("Ha ocurrido un error al obtener el pronóstico del tiempo. Por favor, intenta nuevamente más tarde.");
    }
}
...
async function getWeatherForecast() {
    ...
    try {
        const response = await fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${city}&appid=${apiKey}&lang=es`);
        const data = await response.json();

        // Objeto para almacenar las temperaturas diarias
        ...
        Object.keys(dailyTemperatures).forEach(dateKey => {
            const averageTemperature = dailyTemperatures[dateKey].temperatureSum / dailyTemperatures[dateKey].count;
            const formattedDate = revertDateFormat(dateKey);
            const minTemperature = toCelsius(dailyTemperatures[dateKey].minTemperature);
            const maxTemperature = toCelsius(dailyTemperatures[dateKey].maxTemperature);
            ...
            card.html(`
                ...
                <div class="weather-info">
                    <p class="temp">Temp promedio ${toCelsius(averageTemperature)}°C ${description}</p>
